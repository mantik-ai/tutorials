...
if __name__ == "__main__":
    ...
    datamodule = BigearthnetDataModule(
        ...
        bands=(["B04", "B03", "B02", "B08"] if args.is_4_channels else None),
    )
    ...
    if args.backbone_type == "random":
        ...
        prefix = f"{args.base_encoder}-{args.backbone_type}--" + (
            "_4_channels" if args.is_4_channels else "_3_channels"
        )
        ...
        prefix = (
            f"{model.hparams.base_encoder}-{args.backbone_type}-{model.hparams.data_mode}--"
            + ("_4_channels" if args.is_4_channels else "_3_channels")
        )
    else:
        raise ValueError()

...
class prepare_data:
    ...
    def import_features_labels(self):
        ...
        self.features = pd.read_csv(self.input_path)
        labels = np.array(self.features[self.label_name]).reshape(
...
class RandomForest(prepare_data):
    ...
    def define(self):
        self.model = RandomForestRegressor(
            n_estimators=60, max_depth=20, random_state=settings.random_seed
        )
...
if __name__ == "__main__":
    ...
    (
        ...
        train_labels,
        test_labels,
    ) = prepare_data_inst.split_training_testing(test_features, labels)
